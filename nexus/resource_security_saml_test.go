...
func testAccResourceSecuritySAML() nexus.SAML {
	return nexus.SAML{
...
func TestAccResourceSecuritySAML(t *testing.T) {
	...
	resName := fmt.Sprintf("nexus_security_saml.%s", saml.Name)
...
resource "nexus_security_saml" "saml" {
	idp_metadata                 = "%s"
	entity_id                    = "%s"
	validate_response_signature  = "%s"
	validate_assertion_signature = "%s"
