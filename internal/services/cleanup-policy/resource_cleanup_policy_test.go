...
func TestAccResourceCleanupPolicy_basic(t *testing.T) {
	...
	policy := nexusSchema.CleanupPolicy{
		...
		CriteriaReleaseType:     (*nexusSchema.CriteriaReleaseType)(&releaseType),
		CriteriaAssetRegex:      &assetRegex,
		Retain:                  retain,
		Format:                  nexusSchema.RepositoryFormat(format),
	}
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttr(resourceName, "criteria_release_type", string(*policy.CriteriaReleaseType)),
					...
					resource.TestCheckResourceAttr(resourceName, "format", string(policy.Format)),
				),
			},
		},
	})
