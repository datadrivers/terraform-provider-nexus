...
func ResourceRepositoryTerraformProxy() *schema.Resource {
	return &schema.Resource{
		Description: "Use this resource to create a proxy terraform repository.",

		Create: resourceTerraformProxyRepositoryCreate,
		Delete: resourceTerraformProxyRepositoryDelete,
		Exists: resourceTerraformProxyRepositoryExists,
		Read:   resourceTerraformProxyRepositoryRead,
		Update: resourceTerraformProxyRepositoryUpdate,
...
func getTerraformProxyRepositoryFromResourceData(resourceData *schema.ResourceData) TerraformProxyRepository {
...
func setTerraformProxyRepositoryToResourceData(repo *TerraformProxyRepository, resourceData *schema.ResourceData) error {
...
func resourceTerraformProxyRepositoryCreate(resourceData *schema.ResourceData, m interface{}) error {
	client := m.(*nexus.NexusClient)

	repo := getTerraformProxyRepositoryFromResourceData(resourceData)

	if err := terraformProxyService(client).Create(repo); err != nil {
		return err
	}
	resourceData.SetId(repo.Name)

	return resourceTerraformProxyRepositoryRead(resourceData, m)
}

func resourceTerraformProxyRepositoryRead(resourceData *schema.ResourceData, m interface{}) error {
	client := m.(*nexus.NexusClient)

	repo, err := terraformProxyService(client).Get(resourceData.Id())
	if err != nil {
		return err
	}
	...
	return setTerraformProxyRepositoryToResourceData(repo, resourceData)
}

func resourceTerraformProxyRepositoryUpdate(resourceData *schema.ResourceData, m interface{}) error {
	client := m.(*nexus.NexusClient)

	repoName := resourceData.Id()
	repo := getTerraformProxyRepositoryFromResourceData(resourceData)

	if err := terraformProxyService(client).Update(repoName, repo); err != nil {
		return err
	}
...
func resourceTerraformProxyRepositoryDelete(resourceData *schema.ResourceData, m interface{}) error {
	client := m.(*nexus.NexusClient)
	return terraformProxyService(client).Delete(resourceData.Id())
}

func resourceTerraformProxyRepositoryExists(resourceData *schema.ResourceData, m interface{}) (bool, error) {
	client := m.(*nexus.NexusClient)
