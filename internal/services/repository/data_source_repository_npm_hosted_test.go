...
func testAccDataSourceRepositoryNpmHostedConfig() string {
	return `
data "nexus_repository_npm_hosted" "acceptance" {
	name   = nexus_repository_npm_hosted.acceptance.id
}`
}

func TestAccDataSourceRepositoryNpmHosted(t *testing.T) {
	...
	dataSourceName := "data.nexus_repository_npm_hosted.acceptance"
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccResourceRepositoryNpmHostedConfig(repo) + testAccDataSourceRepositoryNpmHostedConfig(),
