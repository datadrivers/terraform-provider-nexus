...
func testAccDataSourceRepositoryHelmProxyConfig() string {
	return `
data "nexus_repository_helm_proxy" "acceptance" {
	name   = nexus_repository_helm_proxy.acceptance.id
}`
}

func TestAccDataSourceRepositoryHelmProxy(t *testing.T) {
	repoUsingDefaults := repository.HelmProxyRepository{
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccResourceRepositoryHelmProxyConfig(repoUsingDefaults) + testAccDataSourceRepositoryHelmProxyConfig(),
