...
func testAccResourceRepositoryTerraformGroup() repository.RawGroupRepository {
...
func testAccResourceRepositoryTerraformGroupConfig(repo repository.RawGroupRepository) string {
	buf := &bytes.Buffer{}
	tmpl := template.Must(template.New("TerraformGroupRepository").Funcs(acceptance.TemplateFuncMap).Parse(acceptance.TemplateStringRepositoryTerraformGroup))
...
func TestAccResourceRepositoryTerraformGroup(t *testing.T) {
	repoHosted := testAccResourceRepositoryTerraformHosted()
	repo := testAccResourceRepositoryTerraformGroup()
	repo.Group.MemberNames = append(repo.Group.MemberNames, repoHosted.Name)
	resourceName := "nexus_repository_terraform_group.acceptance"
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccResourceRepositoryTerraformHostedConfig(repoHosted) + testAccResourceRepositoryTerraformGroupConfig(repo),
