...
func testAccDataSourceRepositoryCondaProxyConfig() string {
	return `
data "nexus_repository_conda_proxy" "acceptance" {
	name   = nexus_repository_conda_proxy.acceptance.id
}`
}

func TestAccDataSourceRepositoryCondaProxy(t *testing.T) {
	repoUsingDefaults := repository.CondaProxyRepository{
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccResourceRepositoryCondaProxyConfig(repoUsingDefaults) + testAccDataSourceRepositoryCondaProxyConfig(),
