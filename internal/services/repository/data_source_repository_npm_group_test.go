...
func testAccDataSourceRepositoryNpmGroupConfig() string {
	return `
data "nexus_repository_npm_group" "acceptance" {
	name   = nexus_repository_npm_group.acceptance.id
}`
}

func TestAccDataSourceRepositoryNpmGroup(t *testing.T) {
	repoHosted := testAccResourceRepositoryNpmHosted()
	repoGroup := repository.NpmGroupRepository{
	...
	dataSourceName := "data.nexus_repository_npm_group.acceptance"
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccResourceRepositoryNpmHostedConfig(repoHosted) + testAccResourceRepositoryNpmGroupConfig(repoGroup) + testAccDataSourceRepositoryNpmGroupConfig(),
