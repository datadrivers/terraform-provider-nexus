...
func testAccDataSourceRepositoryRGroupConfig() string {
	return `
data "nexus_repository_r_group" "acceptance" {
	name   = nexus_repository_r_group.acceptance.id
}`
}

func TestAccDataSourceRepositoryRGroup(t *testing.T) {
	repoHosted := testAccResourceRepositoryRHosted()
	repoGroup := repository.RGroupRepository{
	...
	dataSourceName := "data.nexus_repository_r_group.acceptance"
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccResourceRepositoryRHostedConfig(repoHosted) + testAccResourceRepositoryRGroupConfig(repoGroup) + testAccDataSourceRepositoryRGroupConfig(),
