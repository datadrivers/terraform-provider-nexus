...
func testAccDataSourceRepositoryRawProxyConfig() string {
	return `
data "nexus_repository_raw_proxy" "acceptance" {
	name   = nexus_repository_raw_proxy.acceptance.id
}`
}

func TestAccDataSourceRepositoryRawProxy(t *testing.T) {
	repoUsingDefaults := repository.RawProxyRepository{
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccResourceRepositoryRawProxyConfig(repoUsingDefaults) + testAccDataSourceRepositoryRawProxyConfig(),
