...
func getAptProxyRepositoryFromResourceData(resourceData *schema.ResourceData) repository.AptProxyRepository {
	...
	if routinngRule, ok := resourceData.GetOk("routing_rule"); ok {
		repo.RoutingRule = tools.GetStringPointer(routinngRule.(string))
		repo.RoutingRuleName = tools.GetStringPointer(routinngRule.(string))
	}
