...
func testAccDataSourceRepositoryNugetGroupConfig() string {
	return `
data "nexus_repository_nuget_group" "acceptance" {
	name   = nexus_repository_nuget_group.acceptance.id
}`
}

func TestAccDataSourceRepositoryNugetGroup(t *testing.T) {
	repoHosted := testAccResourceRepositoryNugetHosted()
	repoGroup := repository.NugetGroupRepository{
	...
	dataSourceName := "data.nexus_repository_nuget_group.acceptance"
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccResourceRepositoryNugetHostedConfig(repoHosted) + testAccResourceRepositoryNugetGroupConfig(repoGroup) + testAccDataSourceRepositoryNugetGroupConfig(),
