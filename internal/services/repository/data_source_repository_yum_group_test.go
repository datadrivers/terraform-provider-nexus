...
func testAccDataSourceRepositoryYumGroupConfig() string {
	return `
data "nexus_repository_yum_group" "acceptance" {
	name   = nexus_repository_yum_group.acceptance.id
}`
}

func TestAccDataSourceRepositoryYumGroup(t *testing.T) {
	repoHosted := testAccResourceRepositoryYumHosted()
	repoGroup := repository.YumGroupRepository{
	...
	dataSourceName := "data.nexus_repository_yum_group.acceptance"
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccResourceRepositoryYumHostedConfig(repoHosted) + testAccResourceRepositoryYumGroupConfig(repoGroup) + testAccDataSourceRepositoryYumGroupConfig(),
