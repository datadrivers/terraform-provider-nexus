...
func testAccDataSourceRepositoryCocoapodsProxyConfig() string {
	return `
data "nexus_repository_cocoapods_proxy" "acceptance" {
	name   = nexus_repository_cocoapods_proxy.acceptance.id
}`
}

func TestAccDataSourceRepositoryCocoapodsProxy(t *testing.T) {
	repoUsingDefaults := repository.CocoapodsProxyRepository{
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccResourceRepositoryCocoapodsProxyConfig(repoUsingDefaults) + testAccDataSourceRepositoryCocoapodsProxyConfig(),
