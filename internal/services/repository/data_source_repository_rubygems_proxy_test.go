...
func testAccDataSourceRepositoryRubygemsProxyConfig() string {
	return `
data "nexus_repository_rubygems_proxy" "acceptance" {
	name   = nexus_repository_rubygems_proxy.acceptance.id
}`
}

func TestAccDataSourceRepositoryRubygemsProxy(t *testing.T) {
	repoUsingDefaults := repository.RubyGemsProxyRepository{
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccResourceRepositoryRubygemsProxyConfig(repoUsingDefaults) + testAccDataSourceRepositoryRubygemsProxyConfig(),
