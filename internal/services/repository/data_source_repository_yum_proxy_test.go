...
func testAccDataSourceRepositoryYumProxyConfig() string {
	return `
data "nexus_repository_yum_proxy" "acceptance" {
	name   = nexus_repository_yum_proxy.acceptance.id
}`
}

func TestAccDataSourceRepositoryYumProxy(t *testing.T) {
	repoUsingDefaults := repository.YumProxyRepository{
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccResourceRepositoryYumProxyConfig(repoUsingDefaults) + testAccDataSourceRepositoryYumProxyConfig(),
