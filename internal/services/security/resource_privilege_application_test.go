		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttr(resName, "actions.3", string(privilege.Actions[4])),
					resource.TestCheckResourceAttr(resName, "actions.3", string(privilege.Actions[5])),
					resource.TestCheckResourceAttr(resName, "actions.3", string(privilege.Actions[6])),
					resource.TestCheckResourceAttr(resName, "actions.3", string(privilege.Actions[7])),
					resource.TestCheckResourceAttr(resName, "actions.3", string(privilege.Actions[8])),
				),
			},
		},
	})
